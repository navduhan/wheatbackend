   ...
   if (body.page){
    ...
    if (body.genes.length>0){
      ...
      if (body.idt==='pathogen'){
        ...
        // console.log(final)
        host_protein =await Results.distinct("Host_Protein",{'Host_Protein':{'$in':body.genes}} ).length
        pathogen_protein =await Results.distinct('Pathogen_Protein',{'Pathgen_Protein':{'$in':body.genes}}).length
      }
